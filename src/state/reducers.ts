...
export const apiKeysReducer = (state: APIKeys | null = null, action: any): APIKeys | null => {
    switch (action.type) {
        case 'apiKeys/assign':
...
export const locationsReducer = (state: WeatherItem[] = [], action: any): WeatherItem[] => {
    switch (action.type) {
        case 'locations/add':
            return [ ...state, { type: 'search', uid: action.uid, location: action.location, data: action.data } ]
...
export const searchReducer = (state: { query: string, results: LocationData[] } = { query: '', results: [] }, action: any): { query: string, results: LocationData[] } => {
    switch (action.type) {
