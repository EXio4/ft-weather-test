import { call, select, put, takeEvery, takeLatest, fork, all, debounce } from 'redux-saga/effects'
import { findLocation, GeocodeResult, getMyLocation, getWeatherData, MyLocationResult, WeatherApiResponse } from '../api'
import { LocationData, PureWeatherData, CurrentWeatherData, WeatherEntry } from '../types';
import { RA_AssignAPIKey, addLocation, RA_UpdateSearchQuery, updateSearchResults, RA_AddLocation, updateLocation } from './actions';
import { ReduxState } from './reducers';

function convertMyLocationResultToLocationData(result: MyLocationResult): LocationData {
    return {
...
function convertApiResponseToPureWeatherData(response: WeatherApiResponse): PureWeatherData {
...
function convertGeocodeResultToLocationData(result: GeocodeResult): LocationData {
    return {
        name: result.name,
        latitude: result.lat,
        longitude: result.lon,
...
function *handleSearchQuery(action: RA_UpdateSearchQuery) {
    try {
        const owmKey: string | null = yield select((state: ReduxState) => state.keys?.openWeatherMap)
        if (!owmKey) {
            console.error('OpenWeatherMap API key not set')
            return
        }
        const results: GeocodeResult[] = yield call(findLocation, owmKey, action.query)
        yield put(updateSearchResults(results.map(convertGeocodeResultToLocationData)))
    } catch (err) {
        console.error('error', err)
    }
}
...
function *handleAddLocation(action: RA_AddLocation) {
    try {
        ...
        const weather: WeatherApiResponse = yield call(getWeatherData, owmKey, action.location.latitude, action.location.longitude)
        yield put(updateLocation(action.uid, null, convertApiResponseToPureWeatherData(weather)))
    } catch (err) {
        console.error('error', err)
    }
}
